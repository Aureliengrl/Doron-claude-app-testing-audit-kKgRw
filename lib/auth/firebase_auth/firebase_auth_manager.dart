...
class FirebaseAuthManager extends AuthManager
  ...
  Future deleteUser(BuildContext context) async {
    try {
      if (!loggedIn) {
        AppLogger.debug('Error: delete user attempted with no logged in user!', 'Debug');
        return;
      }
  ...
  Future updateEmail({
    ...
    try {
      if (!loggedIn) {
        AppLogger.debug('Error: update email attempted with no logged in user!', 'Debug');
        return;
      }
  ...
  Future updatePassword({
    ...
    try {
      if (!loggedIn) {
        AppLogger.debug('Error: update password attempted with no logged in user!', 'Debug');
        return;
      }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      AppLogger.debug('🔄 FirebaseAuthManager: Début authentification ($authProvider)', 'Debug');
      final userCredential = await signInFunc();

      if (userCredential == null) {
        AppLogger.debug('❌ FirebaseAuthManager: userCredential est null', 'Debug');
        return null;
      }

      if (userCredential.user == null) {
        AppLogger.debug('❌ FirebaseAuthManager: userCredential.user est null', 'Debug');
        return null;
      }

      AppLogger.debug('✅ FirebaseAuthManager: User credential obtenu - UID: ${userCredential.user!.uid}', 'Debug');
      AppLogger.debug('🔄 FirebaseAuthManager: Appel maybeCreateUser...', 'Debug');

      await maybeCreateUser(userCredential.user!);

      AppLogger.debug('✅ FirebaseAuthManager: maybeCreateUser terminé', 'Debug');

      return DoronFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      AppLogger.debug('❌ FirebaseAuthManager: FirebaseAuthException - Code: ${e.code}, Message: ${e.message}', 'Debug');
      ...
      AppLogger.debug('❌ FirebaseAuthManager: Exception générique: $e', 'Debug');
